...
class A_1688_Detail():
    def __init__(self):
        ...
        self.headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/61.0.3163.100 Safari/537.36',
                        ...
                        } # 需要登录以后的cookie 可以动态传入 
        self.redis_client = redis.Redis(host="127.0.0.1", port=6379, db=2, password="caonima")
    ...
    async def run_consumer(self, redis_key):
        for md5 in self.redis_client.hgetall(redis_key):  # 可以用while 1    用for的话要在内存中先加载
